        ...
        public async Task<List<Category>> SearchAsync(long hostId, string keyword)
        {
            ...
            using (var sqlConnection = new SqlConnection(_connectionString))
            {
                ...
                while (dr.Read())
                {
                    ...
                    {
                        Id = !dr.IsDBNull(dr.GetOrdinal(nameof(Category.Id))) ? dr.GetInt64(dr.GetOrdinal(nameof(Category.Id))) : 0,
                        Counter = !dr.IsDBNull(dr.GetOrdinal(nameof(Category.Counter))) ? dr.GetInt64(dr.GetOrdinal(nameof(Category.Counter))) : 0
                    };
                    list.Add(category);
                }
                sqlConnection.Close();
